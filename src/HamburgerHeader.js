            ...
            <div ref={menuRef} className={`fixed top-0 right-0 w-64 h-full shadow-lg z-50 transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out`} style={{ backgroundColor: '#234c8b' }}>
                <div className="relative">
                    ...
                    <ul className="flex flex-col items-start justify-start text-white p-8">
                        <li className='mb-2'><Link className="hover-text-custom font-medium" to="/" onClick={closeMenu}>Home</Link></li>
                        <li className='mb-2'><Link className="hover-text-custom font-medium" to="/about" onClick={closeMenu}>About Us</Link></li>
                        <li className='mb-2'><Link className="hover-text-custom font-medium" to="/rush" onClick={closeMenu}>Rush</Link></li>
                        <li className='mb-2'><Link className="hover-text-custom font-medium" to="/members" onClick={closeMenu}>Members</Link></li>
                        <li className='mb-2'><Link className="hover-text-custom font-medium" to="/nationals" onClick={closeMenu}>Nationals</Link></li>
                    </ul>
                </div>
            </div>
